...
CACHE_MODE = False
CACHE_FILE = os.path.join(profile, 'requests_cache')
CACHE_EXPIRE = 604800 # 7 DIAS PRA EXPIRAR
...
if not xbmcvfs.exists(profile):
    try:
        ...
        pass
MENU_URL = 'http://takeoneplus.wuaze.com/servidores.xml'
...
def addon_log(string, level=xbmc.LOGDEBUG):
    if debug == 'true':
        xbmc.log("[plugin.video.neo.tv-{0}]: {1}".format(addon_version, string), LOGINFO)
    else:
        xbmc.log("[plugin.video.neo.tv-{0}]: {1}".format(addon_version, string), level)
...
def urlsolver(url): 
    try: 
        try:
            if not os.path.exists(TRANSLATEPATH('special://home/addons/script.module.resolveurl')):
                try:
            ...
            if not os.path.exists(TRANSLATEPATH('special://home/addons/script.module.urlresolver')):
                try:
                    xbmc.executebuiltin('InstallAddon(script.module.urlresolver)', wait=True)
                except:
                    pass
    ...
    if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo:
        ...
        if index >= 0:
            ...
            if "&mode=19" in mu_playlist[index]:                
                ...
                urltoplay = urlsolver(mu_playlist[index].replace('&mode=19', '').replace(';', ''))
                try:
                    if 'magnet:?xt' in urltoplay and not 'plugin://' in urltoplay:
                        urltoplay = 'plugin://plugin.video.elementum/play?uri=' + urltoplay
                    elif '.torrent' in urltoplay and not 'plugin://' in urltoplay:
                        urltoplay = 'plugin://plugin.video.elementum/play?uri=' + urltoplay
                except:
                    pass
                liz.setPath(urltoplay)
                ...
                try:
                    ...
                    pass
                url = url.replace('&mode=19', '').replace(';', '')
                if not check_iptv(url) and not 'magnet:?xt' in url and not '.torrent' in url and not '.m3u8' in url and not '.ts' in url:
                    urltoplay = urlsolver(url)
                else:
                    urltoplay = url             
                try:
                    if 'magnet:?xt' in urltoplay and not 'plugin://' in urltoplay:
...
def addLink(url, name, iconimage, fanart, description, genre, date, showcontext, playlist, regexs, total, setCookie="", allinfo={}):
    ...
    if allinfo is None or len(allinfo) < 1:
        if get_kversion() > 19:
            info = liz.getVideoInfoTag()
            if not 'magnet' in u and not '.torrent' in u and not 'mode=13' in u:
                info.setTitle(name)
            info.setMediaType('video')
            info.setPlot(description)
            info.setGenres([genre])
    ...
    if showcontext:
        if showcontext == 'fav':
            contextMenu.append(
                ('Remove from %s Favorites'%str(name_addon_info), 'RunPlugin(%s?mode=6&name=%s)'
                 % (sys.argv[0], urllib_parse.quote_plus(name)))
            )
            ...
            contextMenu.append(('Add to %s Favorites'%str(name_addon_info), 'RunPlugin(%s)' % fav_params))
        liz.addContextMenuItems(contextMenu)
    try:
...
if mode is None:
    ...
    dialog = xbmcgui.Dialog()
    ok = dialog.ok(addon_name, 'Parental block enabled')    
